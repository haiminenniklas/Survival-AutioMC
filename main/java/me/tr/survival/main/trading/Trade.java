...
public class Trade {
    ...
    public boolean addItem(Player player, ItemStack newItem) {
       ...
       if(canContinue(true)) {
           int[] playerSlots;
           if(this.sender.getUniqueId().equals(player.getUniqueId())) playerSlots = getSenderSlots();
           else playerSlots = getTargetSlots();

           for(int i = 0; i < playerSlots.length; i++) {
               int slot = playerSlots[i];

               // Let's check if the slot is not taken.
               if(inv.getItem(slot) == null || (inv.getItem(slot) != null && inv.getItem(slot).getType() == Material.AIR)) {
                    success = true;
