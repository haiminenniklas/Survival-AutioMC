...
public class TeleportRequest {
    ...
    public void ask(boolean here) {
        ...
        if(this.getType() == TeleportManager.Teleport.FORCE) accept();
        else {

            if(Main.getEndManager().isInEnd(this.one) || Main.getEndManager().isInEnd(this.two)) {
            ...
            if(here) two.sendMessage(" §7pyytää sinua hänen luokseen!");
            else two.sendMessage(" §7lupaa teleportata luoksesi!");
            two.sendMessage(" ");
    ...
    public void deny(){
        this.denied = true;
        if(this.isExpired()) Chat.sendMessage(two, Chat.Prefix.ERROR, "Teleporttauspyyntö pelaajalta §6" + one.getName() + " §7on jo umpeutunut!");
        else {
    ...
    public void accept() {
        ...
        if(this.isExpired()) Chat.sendMessage(two, Chat.Prefix.ERROR, "Teleporttauspyyntö pelaajalta §a" + one.getName() + " §7on jo umpeutunut!");
        else {
            if(this.getType() == TeleportManager.Teleport.FORCE) this.teleport();
            else {
                if(!this.here) {
    ...
    private void remove() {
        this.expired = true;
        if(this.one == null) throw new IllegalArgumentException("Neither of the players in the Teleportation request cannot be null!");
